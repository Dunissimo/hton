...
export const fetchProjectsReq = createAsyncThunk(
    "projects/getAll",
    async () => getAllProjects()
);

export const createProjectReq = createAsyncThunk(
    "projects/create",
    async (name) => createProject(name)
);
...
export const projectsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(fetchProjectsReq.pending, (state) => {
            state.loading = true;
            state.error = null;
        }).addCase(fetchProjectsReq.fulfilled, (state, {payload}) => {
            state.loading = false;
            state.projects = payload;
        }).addCase(fetchProjectsReq.rejected, (state, {error}) => {
            state.loading = false;
